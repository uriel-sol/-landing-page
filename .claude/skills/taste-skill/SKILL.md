...
## PROJECT OVERRIDES — קוד המפצח (Hebrew RTL landing page) — these rules WIN on conflict

1. **Language & direction:** All copy is Hebrew, layout is RTL (`dir="rtl"`). Use CSS logical properties (`margin-inline-start`, `padding-inline-end`, `text-align: start`) instead of left/right. Mirror direction-dependent icons, arrows, carousels, and slide-in animations.
2. **Typography:** Hebrew has no uppercase — ignore every uppercase/small-caps/eyebrow-label rule. Do NOT apply negative letter-spacing (`tracking-tighter`) to Hebrew text. Latin font stacks (Geist, Inter, etc.) apply only to numerals/Latin fragments; Hebrew text uses the fonts defined in `docs/brand-direction.md` (e.g. Heebo, Assistant, Rubik, Frank Ruhl Libre families).
3. **No external placeholder assets:** Never embed images from picsum.photos, cdn.simpleicons.org, or any third-party CDN. Use local files from `assets/images/` and `assets/icons/` only, or leave a clearly marked local placeholder.
4. **Stack:** This project is a single static `index.html` with vanilla CSS/JS. Do not introduce React, Tailwind, npm packages, or build steps. GSAP/Motion skeletons must be translated to vanilla CSS/JS equivalents.
5. **Authority:** `docs/brand-direction.md` and architect-lead decisions override any styling direction in this skill. Design changes are proposals until architect-lead approves.
