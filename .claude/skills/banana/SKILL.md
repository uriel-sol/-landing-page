...
> **PROJECT POLICY (קוד המפצח) — overrides everything below:** The MCP server is disabled in this installation. Wherever this document says "call the MCP tool" or "run `/banana setup`", instead run the direct API scripts: `python3 ${CLAUDE_SKILL_DIR}/scripts/generate.py` / `scripts/edit.py`, with the API key supplied per-session via the `GOOGLE_AI_API_KEY` environment variable only. Hebrew text must never appear inside generated images (unreliable rendering) — text-free imagery only. See "Setup (project policy)" and "Project Notes" at the end of this file.
...
## Setup (project policy: direct API only)

The MCP setup script was intentionally removed from this installation. Do NOT install or register the `@ycse/nanobanana-mcp` MCP server — this project uses only the direct-API scripts (`scripts/generate.py`, `scripts/edit.py`).

Requires a Google AI API key (free at https://aistudio.google.com/apikey), provided per-session via the `GOOGLE_AI_API_KEY` environment variable. Never write the API key into settings files or commit it to the repository.

## Project Notes — קוד המפצח landing page

- AI models do not render Hebrew (RTL) text reliably inside images. Use this skill ONLY for text-free imagery (backgrounds, textures, photographic/illustrative assets). Any asset containing Hebrew copy must be built in HTML/CSS or a design tool instead.
- Save approved assets into `assets/images/` in the project.
- All generated imagery must pass visual-director → architect-lead review before entering the page.
