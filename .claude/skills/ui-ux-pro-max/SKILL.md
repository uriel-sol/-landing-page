...
1. Stack is static HTML/CSS/JS (single `index.html`) — use only the `html-css` / vanilla guidance; ignore React/Tailwind/shadcn instructions and never run `npx shadcn` or install npm packages.
2. Layout is RTL Hebrew: prefer CSS logical properties, mirror directional patterns, and prefer Hebrew-subset fonts (Heebo, Assistant, Rubik, Alef, Frank Ruhl Libre) over Latin-only pairings.
3. Generated design systems are advisory only — `docs/brand-direction.md` and architect-lead decisions are authoritative. Do not overwrite existing brand decisions with `--persist` output without approval.
4. Do not use the premium/Gemini sub-skills or any feature requiring an API key unless the user explicitly provides one.
